...
void lattice::fillpdb(string filename)//read in a pdb file and fill the lattice with points
{
	...
	while(getline(input, tempLine))
	{
		...
		{
			numElements++;			
			if(head == NULL)//first item create the head
			{
			...
			else
			{
				...
				temp = temp->generateNew(direction,l,w,h);//generate the next points
			}
		}
	}
	//create the offset (what to add to (x,y,z) to make it fit within the bounds of the array)
	...
	temp = head;
	for(int i = 0;i<numElements;i++)//go through the linked list
	{
		//change (x,y,z) to reflex the offest
		temp->setX(temp->getX()+xOffset);
		temp->setY(temp->getY()+yOffset);
		temp->setZ(temp->getZ()+zOffset);
		
		if(a[temp->getX()][temp->getY()][temp->getZ()] == NULL)//make sure no duplicates
		{
			temp -> setNum(i);//sets number
			//place point in the array and in short list
...
int lattice::getDirection(int x,int y,int z, point* temp)//given a point and three coordinates, generate the direction of that point
{
...
bool lattice::shiftRandom()//call shiftPoint on a random point
{
...
bool lattice::shiftPoint(int x, int y, int z)
{
	...
	if(x<5 || x > l-5 || y < 5 || y > w-5 || z < 5 || z > h-5)//too close to the edge
	{
		recenter();
	}
	...
	if(mobile->getDirectionIn()==0 || mobile->getDirectionOut()==0)//end point
	{
		...
		if(newDirection == 6)
			newX--;
		if(a[newX][newY][newZ]==NULL)//change the point if new spot is open
		{
			a[x][y][z] = NULL;//set old spot to null
			
			//change directions in/out from mobile and hinge to reflect new position
			if(mobile->getDirectionIn()==0)
			{
			...
			//change mobile to reflect new position
			mobile->setX(newX);
			mobile->setY(newY);
			mobile->setZ(newZ);
...
void lattice::printLattice(const char* fileOut)//prints data in pdb format to a file
{
	FILE* outputFile;
...
int* lattice::getNewPoints(int rotation, point* current, point* pivot)//given a roation, a current point and a pivot point; returns (x,y,z) of the new point
{
...
int* lattice::generateRandom()//generate a random(x,y,z)
{
	int* toReturn = new int(3);
...
void lattice::copyLattice(lattice* otherLattice)//create an exact copy of another lattice
{
	...
	clearArray();
	for(int i=0;i<numElements;i++)//go through short list and copy all points from otherLattice short list, then copy into array
	{
		shortList[i] -> copyPoint(otherLattice -> shortList[i]);
		x = shortList[i] -> getX();
		y = shortList[i] -> getY();
		z = shortList[i] -> getZ();
		a[x][y][z] = shortList[i];
...
void lattice::clearArray()//make every entry in array a NULL
{
	for(int i=0;i<l;i++)
		for(int j=0;j<w;j++)
			for(int k=0;k<h;k++)
...
void lattice::recenter()
{
	...
	for(int i=0;i<numElements;i++)
	{
		if(shortList[i] -> getX() < xMin)
			xMin = shortList[i] -> getX();
		if(shortList[i] -> getY() < yMin)
			yMin = shortList[i] -> getY();
		if(shortList[i] -> getZ() < zMin)
			zMin = shortList[i] -> getZ();
		if(shortList[i] -> getX() > xMax)
			xMax = shortList[i] -> getX();
		if(shortList[i] -> getY() > yMax)
			yMax = shortList[i] -> getY();
		if(shortList[i] -> getZ() > zMax)
			zMax = shortList[i] -> getZ();
	}
	xOffset = (l-xMin-xMax)/2;
	yOffset = (w-yMin-yMax)/2;
	zOffset = (h-zMin-zMax)/2;
	for(int i=0;i<numElements;i++)
	{
		shortList[i] -> setX(shortList[i]->getX()+xOffset);
		shortList[i] -> setY(shortList[i]->getY()+yOffset);
		shortList[i] -> setZ(shortList[i]->getZ()+zOffset);
	}
}
